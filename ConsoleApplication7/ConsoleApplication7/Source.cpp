...
public:
	...
	int64_t **Attitude;
	   Array(int _size) 
	   {
		   ...
		   Attitude = new int64_t*[_size];
		   //int *buf = (int*)*malloc(sizeof(int)*1024);

		   for (int row = 0; row < _size; row++) {
			   Attitude[row] = new int64_t[tuple_size];
			   Attitude[row][0] = row;
			   for (int col = 1; col < tuple_size; col++) {
				   Attitude[row][col] = rand() % 100000 + rand() % 100000;
...
#pragma omp parallel num_threads(8)
#pragma omp for reduction(+:tuples_connected)
		for (int i = 0; i < sizeof(Att1); i++)
		{
			for (int j = 0; j < sizeof(Att2); j++)
			{
				if (Att1[i][2] == Att2[j][2])
				{
					tuples_connected = tuples_connected + 1;
...
int main()
{
	...
	Array test = Array(10000000);
	Array test2 = Array(10000000);
